...
<div id="bodychild">
    <div id="outercontainer">
        ...
        <div
            ...
            <h2 align="center">充值中心</h2>
            <form align="center" method="post" id="chargeForm">

                <div>
                    <ul>充值号码：<input  name="telno" value="" id="telno" type="text" maxlength="11" placeholder="手机号" style="border: 0;"/></ul>
                    <%--<ul>充值金额：<input type="number" maxlength="2" readonly value="50" /> </ul>--%>
                    <ul>充值金额：
                    <select id="recharge"  style="width: auto">
                        <% if (list!=null && list.size()>0){
                            for (int i =0;i< list.size() ;i++){
                                Items vo = (Items) list.get(i);

                        %>
                        <option name="productId" id="<%=vo.getId()%>" value="<%=vo.getPcost()%>">话费充值卡<%=vo.getPdenomination()%>元，(售价:<%=vo.getPcost()/100%>元)</option>
                        <%
                            }}
                        %>
                    </ul>
                ...
                <input hidden name="productId" value="" id="productId"/>
                <input hidden name="cost" value="" id="cost" />
                <input type="button" value="立即充值" onclick="charge()"/>
            </form>
